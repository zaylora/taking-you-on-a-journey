...
> 实现第一步（Task 0）必须在真实环境验证：`create_agent` 的确切签名与 import（`langchain.agents` vs `langgraph.prebuilt`）；`checkpointer`/`interrupt` 是否如期工作；规避已知 bug（`langgraph-prebuilt==1.0.5` 传 `list[BaseTool]` 会崩，必要时锁 1.0.4）；`state_schema` 是否需自定义以容纳 `day_plans`/`changed_days`/`plan_version`/`budget_check` 等业务字段（`create_agent` 默认 state 仅 `messages`，业务字段需通过自定义 state_schema 或 tool 经 `Command`/`InjectedState` 读写——Task 0 确认机制）。
