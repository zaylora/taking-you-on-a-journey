...
**Files（5 个测试文件，含 controller 审计后补入的 need_routing / m5fix_e2e）:**
- Modify: `backend/tests/test_refine_budget.py`（直接 import `_relax_until_budget` → `_relax_stops`）
- Modify: `backend/tests/test_refine_transport.py`（3 个 refine_request 旧形状 → operations；change_meal 那个还要把 `search_poi` 改 `search_around`）
- Modify: `backend/tests/test_need_routing.py`（change_hotel 路由测试 → `needs_accommodation`）
- Modify: `backend/tests/test_multiturn_refine.py`（e2e 第二轮 refine 现走 LLM 解析，需重打桩 d.build_llm 为 RefinePlan）
- Modify: `backend/tests/test_m5fix_e2e.py`（3 个 e2e refine：重打桩 d.build_llm + change_meal 用 search_around）
- Test: 全量 `cd backend && uv run pytest -q`

**Interfaces:** Consumes Task 3-7 全部产出；无新接口，仅迁移 + 回归。

- [ ] **Step A: test_refine_budget.py（`_relax_until_budget` → `_relax_stops`）**
...
def test_relax_stops_removes_until_fit():
    ...
    assert len(out) < 4
```
Run: `cd backend && uv run pytest tests/test_refine_budget.py -q` → PASS（2 passed）

- [ ] **Step B: test_refine_transport.py（3 处 refine_request + change_meal 的 search 通道）**

只改三处 `refine_request`（其余 helper/断言不动），并改 (c) 的 fake_amap 通道：
- (a) `test_reorder_rebuilds_transport_segments` 的 refine_request → `{"operations": [{"op": "reorder", "day": 1, "strategy": "reverse"}]}`（必须 reverse，断言要 [C,B,A] 倒序）
- (b) `test_relax_no_dangling_transport` 的 refine_request → `{"operations": [{"op": "set_pace", "day": 1, "direction": "relax"}]}`
- (c) `test_change_meal_updates_surrounding_transport`：
  - 把 `fake_amap["search_poi"] = [...]` 改为 `fake_amap["search_around"] = [...]`（新 replace_poi handler 走 `search_around` 圆心检索，不再用全城 `search_poi`）。
  - refine_request → `{"operations": [{"op": "replace_poi", "day": 1, "kind": "meal", "query": "火锅", "selector": {"by": "ordinal", "kind": "meal", "index": 0}}]}`
  - 该天有 1 个 meal（旧饭馆）→ selector 命中 index 0 → 替换为新火锅；断言（meal 名、交通段 to=新火锅）不变。
Run: `cd backend && uv run pytest tests/test_refine_transport.py -q` → PASS（3 passed）

- [ ] **Step C: test_need_routing.py（change_hotel 路由）**

`route_after_plan` 现读 `needs_accommodation`，不再认 `op`。只改 `test_refine_change_hotel_goes_accommodation`：
```python
...
```
另外两个（relax/reorder）路由测试不依赖 op（routing 已忽略 op），仍 PASS，但顺手删掉其 refine_request 里无意义的 `"op": ...` 键以免误导（保留 `needs_budget_recheck`）。
Run: `cd backend && uv run pytest tests/test_need_routing.py -q` → PASS（5 passed）

- [ ] **Step D: test_multiturn_refine.py（第二轮 refine 重打桩 d.build_llm）**

第二轮 "第二天太赶了，少安排一个景点" 现走 LLM 解析；`_stub_initial_plan` 把 `dispatch_agent.build_llm` 固定为返回 NormalizedReq，会让 refine 解析拿到 NormalizedReq（`.operations` AttributeError）。在 `test_second_turn_relaxes_only_target_day` 里、第二个 `client.post` **之前**插入重打桩：
```python
...
```
其余断言（plan_version==2、patch=={plan_version:2,changed_days:[2]}、day1 不动、day2 变短）不变。
Run: `cd backend && uv run pytest tests/test_multiturn_refine.py -q` → PASS

- [ ] **Step E: test_m5fix_e2e.py（3 个 e2e refine：重打桩 + search 通道）**

每个 refine e2e 在第二个 `client.post` 之前重打桩 `dispatch_agent.build_llm` 为对应 RefinePlan（import 同 Step D）：
- `test_change_meal_only_target_day_and_runs_budget`：把 `fake_amap["search_poi"] = [蜀大侠火锅]` 改为 `fake_amap["search_around"] = [蜀大侠火锅]`。换餐 op 取决于初始 e2e 计划里 day1 是否已有 meal（初始规划时 search_around 默认空，day1 很可能无 meal）：**先跑一次**观察——若 day1 无 meal，用 `add_poi`（kind=meal）；若有 meal，用 `replace_poi`（kind=meal, selector ordinal meal 0）。RefinePlan 相应为 `[Operation(op="add_poi", day=1, kind="meal", query="火锅")]` 或 replace 版。断言 `meals == ["蜀大侠火锅"]`、`changed_days==[1]`——若实测语义合理但数量/位置有别，按**新设计的正确行为**更新断言并在报告里说明（不得为凑绿而弱化断言）。
- `test_reorder_skips_accommodation_and_budget`：重打桩为 `[Operation(op="reorder", day=1, strategy="reverse")]`。reorder → needs_accommodation/needs_budget_recheck 均 False → summarize，不碰 accommodation（boom 不触发）。断言 changed_days==[1] 不变。
- `test_change_budget_updates_limit`：重打桩为 `[Operation(op="set_budget", amount=1500)]`。断言 budget.limit==1500 不变。
Run: `cd backend && uv run pytest tests/test_m5fix_e2e.py -q` → PASS（3 passed）

- [ ] **Step F: 全量回归**

Run: `cd backend && uv run pytest -q`
Expected: 全绿（仅余既有第三方 Starlette/Swig 弃用告警）。若有遗漏的旧形状/已删符号引用导致挂测，按本任务同款方式修正。
...
- §7 测试：翻转/删除旧 dispatch 用例(T6)、重写 test_refine_node(T3)、新增复合/set_region/selector/空澄清/诚实 notes(T2-T7)、审计迁移 budget/transport/need_routing/multiturn/m5fix_e2e(T8，controller 审计后补入 need_routing + m5fix_e2e) ✓
