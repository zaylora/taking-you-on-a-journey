...
```
backend/app/
  agent/        # ReAct agent：build、state、prompt、reducers
  api/          # FastAPI 路由（chat、sessions）+ chat_stream（SSE 桥接）
  middleware/   # Agent 中间件：当前时间注入、大工具结果落盘
  tools/        # 能力层：actions（@tool）/ planning（纯计算）/ clients（外部服务客户端）
...
docs/superpowers/ 改动记录 # specs/（设计文档）、plans/（实现计划）
plan/           # 每次改动的记录文档（见下）
```
...
- 纯计算（预算核算、路线优化、聚类、diff）放 `tools/planning/`，写成无副作用纯函数，单独测。
...
- 大工具结果落盘由 `ToolResultPersistenceMiddleware` 统一处理，工具本身返回原始业务结果；需要原文时通过 `read_persisted_tool_result` 分页读取。
...
每次完成任务或实现改动，**必须**在 `docs/` 下创建记录：

- 文件夹：`docs/YYYYMMDD_<任务简述>/`，例如 `docs/20260628_xhs_source_links/`。
- 内含 `README.md`，包含：**任务目标**、**改动文件清单**（建议用表格）、**改动详情**（改了什么、为何这样改）、**测试结果**、**相关讨论**（重要设计决策）。
- 在 `docs/README.md` 文件中写入位置和简要概述改动内容
